# CapricoinPlus Service

The CapricoinPlus Service is a Node.js interface to [CapricoinPlus Core](https://github.com/capricoinplus/capricoinplus-core) for querying information about the capricoinplus block chain. It will manage starting and stopping `capricoinplusd` or connect to several running `capricoinplusd` processes. It uses an instance of a CapricoinPlus with additional indexes enabled for querying information about addresses and blocks. Results are cached for performance and there are several additional API methods added for common queries.

## Configuration

The default configuration will include a "spawn" configuration in "capricoinplusd". This defines the location of the block chain database and the location of the `capricoinplusd` daemon executable. The below configuration points to a local clone of `capricoinplus`, and will start `capricoinplusd` automatically with your Node.js application.
...
It's also possible to connect to separately managed `capricoinplusd` processes with round-robin quering, for example:
